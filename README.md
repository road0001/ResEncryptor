...
- 遍历生成的文件获取文件序列，先打乱顺序，然后使用下列规则重命名并生成映射关系的文件序列，将其保存到解密配置中。
  - -1、原始文件名：注意：如果使用无EXE的单文件或双文件模式，将被自动添加随机后缀，防止创建同名文件夹失败导致无法解压。
...
- 根据不同模式，将进行如下处理：
  ...
  - 负值、无EXE模式：不创建EXE文件，需要下载本工具并将其拖到EXE上进行解压。
    - -0、-1、-2分别对应正值的0、1、2模式。
...
- 无EXE模式的算法和前面的算法基本相同，不同之处仅在于生成的主文件不再整合进EXE文件中，而是单独的文件。您需要来这里下载本工具，然后将打包的文件拖到EXE图标上进行解压。
- 为了防止无法创建同名文件夹导致解压失败的问题，因此会在生成的文件名后面加随机后缀，请勿将其删除。
- 无EXE单文件模式不受4GB总大小限制。
...
- Python版本不低于3.11。
  - 如果您需要在Windows 7等系统运行本工具，请使用Python 3.8打包。
